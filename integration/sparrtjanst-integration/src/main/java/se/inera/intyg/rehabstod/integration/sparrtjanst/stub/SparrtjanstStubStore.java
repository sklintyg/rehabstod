...
/** Simple embedded redis cache store for blocks. Created by marced on 2018-10-01. */
@Component
...
public class SparrtjanstStubStore {
  ...
  public boolean isBlockedAtDate(
      String personId,
    ...
    return getAllForPerson(personId).stream()
        .filter(
            blockData -> {
              boolean isVg =
                  Strings.isNullOrEmpty(blockData.getVardGivareId())
                      || blockData.getVardGivareId().equals(vardGivareId);
              boolean isVe =
                  Strings.isNullOrEmpty(blockData.getVardEnhetId())
                      || blockData.getVardEnhetId().equals(vardEnhetId);
              return isVg && isVe;
            })
        .anyMatch(
            blockData ->
                isWithinInterval(blockData, queryDateFrom)
                    || isWithinInterval(blockData, queryDateTo));
  }
  ...
  private boolean isWithinInterval(BlockData blockData, LocalDate queryDate) {
    ...
    return queryDate.isEqual(blockData.getBlockFrom())
        || queryDate.isEqual(blockData.getBlockTo())
        || (queryDate.isAfter(blockData.getBlockFrom())
            && queryDate.isBefore(blockData.getBlockTo()));
  }
}
