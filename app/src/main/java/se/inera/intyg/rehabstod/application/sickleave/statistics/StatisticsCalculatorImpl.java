...
import se.inera.intyg.rehabstod.application.exceptions.SjukfallServiceException;
...
import se.inera.intyg.rehabstod.application.sjukfall.dto.DiagnosGruppStat;
import se.inera.intyg.rehabstod.application.sjukfall.dto.GenderStat;
import se.inera.intyg.rehabstod.application.sjukfall.dto.SickLeaveDegreeStat;
import se.inera.intyg.rehabstod.application.sjukfall.dto.SickLeaveLengthStat;
...
public class StatisticsCalculatorImpl implements StatisticsCalculator {
  ...
  public static final DiagnosGrupp NON_MATCHING_GROUP =
      new DiagnosGrupp("", new ArrayList<>(), "Utan giltig diagnoskod");

  private List<DiagnosGrupp> diagnosGrupper;

  @Autowired DiagnosGruppLoader diagnosGruppLoader;
  ...
  public SickLeaveSummary getSickLeaveSummary(List<SjukfallEnhet> sickLeave) {
    ...
    final var maleDiagnosisGroups =
        calculateGroupStatistics(filterSickLeavesByGender(sickLeave, Gender.M));
    final var femaleDiagnosisGroups =
        calculateGroupStatistics(filterSickLeavesByGender(sickLeave, Gender.F));

    final var sickLeaveDegrees = filterEmptyPercentages(calculateSickLeaveDegrees(sickLeave));
    final var maleSickLeaveDegrees =
        filterEmptyPercentages(
            calculateSickLeaveDegrees(filterSickLeavesByGender(sickLeave, Gender.M)));
    final var femaleSickLeaveDegrees =
        filterEmptyPercentages(
    ...
    final var countMaleSickLeaveDegrees =
        countSickLeaveDegrees(filterSickLeavesByGender(sickLeave, Gender.M));
    final var countFemaleSickLeaveDegrees =
        countSickLeaveDegrees(filterSickLeavesByGender(sickLeave, Gender.F));

    final var sickLeaveLength = calculateSickLeaveLength(sickLeave);
    final var maleSickLeaveLength =
        calculateSickLeaveLength(filterSickLeavesByGender(sickLeave, Gender.M));
    final var femaleSickLeaveLength =
        calculateSickLeaveLength(filterSickLeavesByGender(sickLeave, Gender.F));
  ...
  private List<SjukfallEnhet> filterOnSickLeaveLength(
      List<SjukfallEnhet> sickLeaves, Predicate<SjukfallEnhet> filter) {
    return sickLeaves.stream().filter(filter).collect(Collectors.toList());
  }
  ...
  private List<SickLeaveLengthStat> calculateSickLeaveLength(List<SjukfallEnhet> sickLeaves) {
    ...
    final var secondLimit =
        filterOnSickLeaveLength(
            sickLeaves, (sickLeave) -> sickLeave.getDagar() >= 90 && sickLeave.getDagar() <= 180);
    final var thirdLimit =
        filterOnSickLeaveLength(
            sickLeaves, (sickLeave) -> sickLeave.getDagar() <= 365 && sickLeave.getDagar() > 180);
  ...
  private SickLeaveLengthStat getStatForSickLeaveLength(
      List<SjukfallEnhet> sickLeaves, int id, int totalCount) {
  ...
  private List<SickLeaveDegreeStat> calculateSickLeaveDegrees(List<SjukfallEnhet> sjukfall) {
    Map<Integer, List<SjukfallEnhet>> byGrad =
        sjukfall.stream().collect(Collectors.groupingBy(SjukfallEnhet::getAktivGrad));
        ...
        .map(
            entry ->
                new SickLeaveDegreeStat(
                    ...
                    "" + entry.getKey() + " %",
                    entry.getValue().size(),
                    calculatePercentage(entry.getValue().size(), sjukfall.size())))
  ...
  private List<GenderStat> calculateGenderStat(List<SjukfallEnhet> sjukfall) {
    Map<Gender, List<SjukfallEnhet>> byGender =
        sjukfall.stream().collect(Collectors.groupingBy(s -> s.getPatient().getKon()));
    ...
    return Arrays.asList(
        new GenderStat(
            Gender.F, womenTotal, calculatePercentage(womenTotal, (womenTotal + menTotal))),
        new GenderStat(Gender.M, menTotal, calculatePercentage(menTotal, (womenTotal + menTotal))));
  }

  private List<SjukfallEnhet> filterSickLeavesByGender(
      List<SjukfallEnhet> sickLeaves, Gender gender) {
