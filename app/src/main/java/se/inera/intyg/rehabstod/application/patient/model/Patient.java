 ...
 */
package se.inera.intyg.rehabstod.application.patient.model;
...
public class Patient {
  ...
  public Patient(String id, String namn) {
    ...
    if (this.id.matches("^(19|20)[0-9]{6}[-+]?[0-9]{4}$")) {
      ...
      normalizedPnr =
          getCenturyFromYearAndSeparator(this.id) + this.id.replace("-", "").replace("+", "");
      kon = Gender.getGenderFromString(normalizedPnr.substring(GENDER_START, GENDER_END));
      alder = getPatientAge(normalizedPnr);
    } else {
  ...
  private int getPatientAge(String patientId) {
    ...
    try {
      ...
      LOG.error(
          "patientId '"
              + patientId
              + "' cannot be parsed as a date for age-calculation (adjusting for samordningsnummer did not help)",
          e);
      age = 0;
    }
  ...
  private String getCenturyFromYearAndSeparator(String personnummer) {
    ...
    final int yearFromPersonnummer =
        Integer.parseInt(personnummer.substring(yearStartIndex, yearStartIndex + 2));
