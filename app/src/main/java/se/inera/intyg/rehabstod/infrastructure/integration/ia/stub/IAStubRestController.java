...
@Profile({"dev", "ia-stub"})
public class IAStubRestController {

  @Autowired private CacheManager cacheManager;

  @GetMapping("/")
  public ResponseEntity<List<Banner>> getBanners() {
    return ResponseEntity.ok(queryCache());
  }
  ...
  public ResponseEntity<Void> addBanner(@RequestBody Banner banner) {
    ...
    banners.add(banner);
    getCache().put(IaCacheConstants.CACHE_KEY, banners.toArray(new Banner[0]));
  ...
  private Cache getCache() {
    return cacheManager.getCache(IaCacheConstants.IA_CACHE_NAME);
  }

  private List<Banner> queryCache() {
    Cache cache = getCache();
    Banner[] banners = cache.get(IaCacheConstants.CACHE_KEY, Banner[].class);
    if (banners == null) {
