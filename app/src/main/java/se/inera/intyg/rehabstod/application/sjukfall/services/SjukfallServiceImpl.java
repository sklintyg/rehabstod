 ...
 */
package se.inera.intyg.rehabstod.application.sjukfall.services;
...
import se.inera.intyg.rehabstod.infrastructure.integration.it.service.IntygstjanstIntegrationService;
...
public class SjukfallServiceImpl implements SjukfallService {
  ...
  public SjukfallPatientResponse getByPatient(
    ...
    return new SjukfallPatientResponse(
        rehabstodSjukfall, result.getSjfMetaData(), srsError, qaInfoError);
  }

  // CHECKSTYLE:OFF ParameterNumber
  private FilteredSjukFallByPatientResult getFilteredSjukfallByPatient(
      String vardgivareId,
      String enhetsId,
      String lakareId,
      String patientId,
    ...
    Preconditions.checkArgument(
        !Strings.isNullOrEmpty(vardgivareId), "vardenhetId may not be null or empty");
    Preconditions.checkArgument(
        !Strings.isNullOrEmpty(enhetsId), "enhetsId may not be null or empty");
    Preconditions.checkArgument(
        !Strings.isNullOrEmpty(lakareId), "lakareId may not be null or empty");
    Preconditions.checkArgument(
    ...
    // Map to sjukfall DTO
    List<IntygData> data =
        intygsData.stream().map(o -> intygstjanstMapper.map(o)).collect(Collectors.toList());
    ...
    data.forEach(
        intygData ->
            intygAccessMetaData.put(
                intygData.getIntygId(),
  ...
  private void addSjfMetaDataItemToMap(
      String id,
      String namn,
      SjfMetaDataItemType type,
      IntygAccessControlMetaData iacm,
      Map<String, SjfMetaDataItem> dataMap) {
  ...
  private boolean checkForConsent(
    ...
    try {
      ...
      haveConsent =
          samtyckestjanstIntegrationService.checkForConsent(
              patientId, lakareId, vardgivareId, enhetsId);
      if (haveConsent) {
      ...
      // INTYG-8405: Om Samtyckestjänsten inte är tillgänglig ska efterfrågad information hanteras
      // som spärrad.
  ...
  private void pdlLogConsentExistRead(String patientId, String enhetsId) {
    final Personnummer patientPersonnummer =
        Personnummer.createPersonnummer(patientId)
            .orElseThrow(
                () ->
                    new IllegalArgumentException(
                        "Could not parse passed personnummer: " + patientId));
    ...
    if (!isInStore) {
      ...
      PDLActivityStore.addActivityToStore(
          enhetsId, patientId, readActivityType, resourceTypeSamtycke, user.getStoredActivities());
    }
  }

  private void declineAccessToSjfData(
      Map<String, IntygAccessControlMetaData> intygAccessMetaData,
      List<IntygData> intygOnOtherUnits) {
    intygOnOtherUnits.forEach(
        intygData -> {
          intygAccessMetaData.get(intygData.getIntygId()).setSparr(true);
        });
  }

  private void decorateWithBlockStatus(
      String vardgivareId,
      String enhetsId,
      String lakareId,
      String patientId,
    ...
    try {
      ...
      sparrtjanstIntegrationService.decorateWithBlockStatus(
          vardgivareId, enhetsId, lakareId, patientId, intygAccessMetaData, intygOnOtherUnits);
    } catch (Exception e) {
      LOG.error("INTEGRATION_BLOCKING_SERVICE: Fatal error - message is '{}'", e.getMessage());

      // INTYG-8405: Om Spärrtjänsten inte är tillgänglig ska efterfrågad information hanteras som
      // spärrad.
      declineAccessToSjfData(intygAccessMetaData, intygOnOtherUnits);
  ...
  private List<IntygData> filterByAcessMetaData(
      List<IntygData> data,
      Map<String, IntygAccessControlMetaData> intygAccessMetaData,
      boolean haveConsent) {
    return data.stream()
        .filter(
            intygData ->
                shouldInclude(intygAccessMetaData.get(intygData.getIntygId()), haveConsent))
        .collect(Collectors.toList());
  }

  private List<IntygData> filterByOtherUnitsOnly(
      String currentVardgivarHsaId, String currentVardenhetHsaId, List<IntygData> intygList) {
    ...
    if (aktivtSjukfall.getSlut().isAfter(parameters.getAktivtDatum())) {
      aktivtSjukfall
          ...
          .forEach(
              sjukfallIntyg ->
                  intygAccessMetaData
                      .get(sjukfallIntyg.getIntygId())
                      .setBidrarTillAktivtSjukfall(true));
    }
  }

  private void updateSjfMetaData(
      final SjfMetaData sjfMetaData,
  ...
  private boolean shouldBeIncludedInCalculationOfSjukfall(
      Collection<String> vgHsaIds, Collection<String> veHsaIds, IntygData intygData) {
    return vgHsaIds.contains(intygData.getVardgivareId())
        || veHsaIds.contains(intygData.getVardenhetId());
  }
  ...
  private boolean isIntygIssuedOnEnhetOrMottagning(Vardenhet vardenhet, String intygEnhetsId) {
    ...
    return mottagningar.stream().anyMatch(unit -> unit.getId().equals(intygEnhetsId));
  }
}
