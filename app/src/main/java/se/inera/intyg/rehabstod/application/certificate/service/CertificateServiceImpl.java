 ...
 */
package se.inera.intyg.rehabstod.application.certificate.service;
...
import se.inera.intyg.rehabstod.application.sickleave.service.SickLeavePersonFilterService;
...
public class CertificateServiceImpl implements CertificateService {
  ...
  public CertificateServiceImpl(
      IntygstjanstRestIntegrationService restIntegrationService,
  ...
  @Override
  public GetLUCertificatesForCareUnitResponse getLUCertificatesForCareUnit(
      GetLUCertificatesForCareUnitRequest request) {
    ...
    var diagnosedCertificateList =
        restIntegrationService.getDiagnosedCertificatesForCareUnit(
            unitIds, certTypesToQuery, request.getFromDate(), request.getToDate(), doctorIds);
    ...
    return new GetLUCertificatesForCareUnitResponse(
        luCertificateList, !hasDecoratedWithUnansweredCommunication);
  }

  private List<DiagnosedCertificate> filterOnDiagnoses(
      List<DiagnosedCertificate> diagnosedCertificates, List<String> diagnoses) {
  ...
  private List<DiagnosedCertificate> filterOnAge(
      List<DiagnosedCertificate> diagnosedCertificates, int fromAge, int toAge) {
  ...
  private List<LUCertificate> filterOnQuestionAndAnswers(
      GetLUCertificatesForCareUnitRequest request, List<LUCertificate> luCertificateList) {
    var qas = request.getQuestionAndAnswers();
    if (qas > 0) {
      switch (qas) {
        case 1: // Only show certificates without unanswered complement requests and questions
          luCertificateList =
              luCertificateList.stream()
                  .filter(c -> c.getUnAnsweredComplement() == 0 && c.getUnAnsweredOther() == 0)
                  .collect(Collectors.toList());
          break;
        ...
        case 3: // Only show certificates with unanswered complement requests
          luCertificateList =
              luCertificateList.stream()
                  .filter(c -> c.getUnAnsweredComplement() > 0)
                  .collect(Collectors.toList());
          break;
        case 4: // Only show certificates with unanswered questions
          luCertificateList =
              luCertificateList.stream()
                  .filter(c -> c.getUnAnsweredOther() > 0)
                  .collect(Collectors.toList());
          break;
        default:
          break;
  ...
  private boolean filterOnText(LUCertificate c, String searchText) {
    ...
    if (patient != null) {
      ...
      if (patient.getKon() != null
          && containsIgnoreCase(patient.getKon().getDescription(), searchText)) {
        return true;
      }
      if (patient.getAlder() > -1
          && containsIgnoreCase(String.format("%d år", patient.getAlder()), searchText)) {
        return true;
      }
    }

    if (c.getDiagnosis() != null) {
      if (c.getDiagnosis().getIntygsVarde() != null
          && containsIgnoreCase(c.getDiagnosis().getIntygsVarde(), searchText)) {
        return true;
      }
      if (c.getDiagnosis().getBeskrivning() != null
          && containsIgnoreCase(c.getDiagnosis().getBeskrivning(), searchText)) {
        return true;
      }
    }
    ...
    if (biDiagnoses != null && !biDiagnoses.isEmpty()) {
      for (var diagnosis : biDiagnoses) {
        if (diagnosis.getIntygsVarde() != null
            && containsIgnoreCase(diagnosis.getIntygsVarde(), searchText)) {
          return true;
        }
        if (diagnosis.getBeskrivning() != null
            && containsIgnoreCase(diagnosis.getBeskrivning(), searchText)) {
          return true;
        }
      }
    }
    ...
    if (c.getSigningTimeStamp() != null
        && containsIgnoreCase(c.getSigningTimeStamp().toLocalDate().toString(), searchText)) {
      return true;
    }
    ...
    return c.getDoctor() != null
        && c.getDoctor().getNamn() != null
        && containsIgnoreCase(c.getDoctor().getNamn(), searchText);
  }
  ...
  private boolean filterOnDiagnoses(
      DiagnosedCertificate diagnosedCertificate, List<String> diagnoseGroupList) {
  ...
  /**
   * Creates a numeric representation of diagnose and the selected diagnoseGroup and checks if the
   * diagnose is within the range of the diagnoseGroup.
   *
   * <p>The diagnoseGroup should be in the format of this example: "A00-B99".
   *
  ...
  private boolean belongsToDiagnoseGroup(String diagnose, String diagnoseGroup) {
    ...
    try {
      ...
      int startValue =
          (Character.getNumericValue(splits[0].toCharArray()[0]) * 100)
              + Integer.parseInt(splits[0].substring(1, 3));
      int endValue =
          (Character.getNumericValue(splits[1].toCharArray()[0]) * 100)
              + Integer.parseInt(splits[1].substring(1, 3));

      // get int representation of diagnose code (first 3 characters only)
      int diagnoseValue =
          (Character.getNumericValue(diagnose.toCharArray()[0]) * 100)
              + Integer.parseInt(diagnose.substring(1, 3));

      return diagnoseValue >= startValue && diagnoseValue <= endValue;
    } catch (NumberFormatException exception) {
      // Likely due to an invalid diagnose code containing other values than a starting char
      // followed by numbers. i.e. "B65" or "B651"
      return false;
    }
  }
  ...
  public GetDoctorsForUnitResponse getDoctorsForUnit() {
    ...
    final var doctorIds =
        restIntegrationService.getSigningDoctorsForUnit(unitIds, Arrays.asList(LU_TYPE_LIST));
  ...
  public GetLUCertificatesForPersonResponse getLUCertificatesForPerson(String personId) {
    ...
    var diagnosedCertificateList =
        restIntegrationService.getDiagnosedCertificatesForPerson(
            personId, Arrays.asList(LU_TYPE_LIST), unitIds);

    var luCertificateList =
        transformDiagnosedCertificatesToLUCertificates(diagnosedCertificateList);
    final var hasDecoratedWithUnansweredCommunication =
        unansweredCommunicationDecoratorService.decorateLuCertificates(luCertificateList);
    ...
    return new GetLUCertificatesForPersonResponse(
        luCertificateList, !hasDecoratedWithUnansweredCommunication);
  }
  ...
  public GetAGCertificatesForPersonResponse getAGCertificatesForPerson(String personId) {
    ...
    final var sickLeaveCertificateList =
        restIntegrationService.getSickLeaveCertificatesForPerson(
            personId, Arrays.asList(AG_TYPE_LIST), unitIds, doctorIds);

    final var agCertificateList =
        transformSickLeaveCertificatesToAGCertificates(sickLeaveCertificateList);
  ...
  private void pdlLogCertificatesForPerson(
      String personId, String unitId, Map<String, List<PDLActivityEntry>> storedActivities) {
    var patientId =
        Personnummer.createPersonnummer(personId)
            .orElseThrow(
                () -> new IllegalArgumentException("Could not parse passed personId: " + personId));
    ...
    var isInStore =
        PDLActivityStore.isActivityInStore(
            unitId, personId, readActivityType, resourceTypeCertificate, storedActivities);
  ...
  private void pdlLogLUCertificatesForCareUnit(List<LUCertificate> luCertificateList) {
    ...
    List<LUCertificate> luCertificateToLog =
    ...
    if (luCertificateToLog.isEmpty()) {
      ...
      logService.logCertificate(
          luCertificateToLog, readActivityType, resourceTypeCertificate, storedActivities);
  ...
  private List<AGCertificate> transformSickLeaveCertificatesToAGCertificates(
      List<SickLeaveCertificate> sickLeaveCertificateList) {

    return sickLeaveCertificateList.stream()
        .filter(this::commonFilter)
        .map(this::convertSickLeaveCertificateToLUCertificate)
        .sorted(Comparator.comparing(AGCertificate::getStart).reversed())
        .collect(Collectors.toList());
  }

  private AGCertificate convertSickLeaveCertificateToLUCertificate(
    ...
    var careProvider =
        hsaOrganizationsService.getVardgivareInfo(sickLeaveCertificate.getCareProviderId());
    var workCapacityList = sickLeaveCertificate.getWorkCapacityList();
    var startDate =
        workCapacityList.stream()
            .min(Comparator.comparing(WorkCapacity::getStartDate))
            .get()
            .getStartDate();
    var endDate =
        workCapacityList.stream()
            .max(Comparator.comparing(WorkCapacity::getEndDate))
            .get()
            .getEndDate();

    return AGCertificate.builder()
        .certificateId(sickLeaveCertificate.getCertificateId())
        ...
        .patient(
            new Patient(
                sickLeaveCertificate.getPersonId(), sickLeaveCertificate.getPatientFullName()))
        .doctor(
            new Lakare(
                sickLeaveCertificate.getPersonalHsaId(),
                sickLeaveCertificate.getPersonalFullName()))
        ...
        .degree(
            workCapacityList.stream()
                .sorted(Comparator.comparing(WorkCapacity::getStartDate))
                .map(WorkCapacity::getReduction)
                .collect(Collectors.toList()))
        .occupation(Arrays.asList(sickLeaveCertificate.getOccupation().split(",")))
        .build();
  }
    ...
    return diagnosedCertificateList.stream()
        ...
        .map(
            diagnosedCertificate ->
                LUCertificate.builder()
                    .certificateId(diagnosedCertificate.getCertificateId())
                    .certificateType(
                        translateCertificateTypeName(diagnosedCertificate.getCertificateType()))
                    .careProviderId(diagnosedCertificate.getCareProviderId())
                    .careProviderName(
                        careProviderMap.get(diagnosedCertificate.getCareProviderId()).getNamn())
                    ...
                    .patient(
                        new Patient(
                            diagnosedCertificate.getPersonId(),
                            diagnosedCertificate.getPatientFullName()))
  ...
  private Map<String, Lakare> getDoctorMap(List<DiagnosedCertificate> diagnosedCertificateList) {
    ...
    diagnosedCertificateList.forEach(
        diagnosedCertificate -> {
  ...
  private Map<String, Vardgivare> getCareProviderMap(
      List<DiagnosedCertificate> diagnosedCertificateList) {
  ...
  private List<LUCertificate> transformDiagnosedCertificatesToLUCertificates(
    ...
    return diagnosedCertificateList.stream()
        .filter(this::commonFilter)
        .map(this::convertDiagnosedCertificateToLUCertificate)
        .collect(Collectors.toList());
  }
  ...
  private LUCertificate convertDiagnosedCertificateToLUCertificate(
    ...
    var careProvider =
        hsaOrganizationsService.getVardgivareInfo(diagnosedCertificate.getCareProviderId());

    return LUCertificate.builder()
        .certificateId(diagnosedCertificate.getCertificateId())
        ...
        .patient(
            new Patient(
                diagnosedCertificate.getPersonId(), diagnosedCertificate.getPatientFullName()))
        .doctor(
            new Lakare(
                diagnosedCertificate.getPersonalHsaId(),
                diagnosedCertificate.getPersonalFullName()))
        .diagnosis(getDiagnosis(diagnosedCertificate.getDiagnoseCode()))
        .biDiagnoses(getDiagnosisList(diagnosedCertificate.getSecondaryDiagnoseCodes()))
        .build();
  }
  ...
  private Diagnos getDiagnosis(String code) {
    if (code != null) {
      ...
      return diagnosFactory.getDiagnos(
          diagnoseCode.getOriginalCode(), diagnoseCode.getCleanedCode(), diagnoseCode.getName());
    } else {
      return null;
    }
  }
