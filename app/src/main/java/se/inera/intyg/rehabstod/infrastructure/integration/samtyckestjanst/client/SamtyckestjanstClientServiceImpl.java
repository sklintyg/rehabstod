 ...
 */
package se.inera.intyg.rehabstod.infrastructure.integration.samtyckestjanst.client;
...
public class SamtyckestjanstClientServiceImpl implements SamtyckestjanstClientService {
  ...
  public CheckConsentResponseType checkConsent(
    ...
    checkConsentType.setPatientId(
        SamtyckestjanstUtil.buildIITypeForPersonOrSamordningsnummer(personnummer));
    checkConsentType.setAccessingActor(
        SamtyckestjanstUtil.buildAccessingActorType(vgHsaId, veHsaId, userHsaId));

    try {
      return checkConsentservice.checkConsent(logicalAddress, checkConsentType);
    } catch (Exception e) {
      LOG.error(
          "CONSENT_SERVICE: Failure when calling Samtyckestjänsten and the checkConsent service.");
  ...
  @Override
  @PerformanceLogging(
      eventAction = "register-extended-consent",
  ...
  public RegisterExtendedConsentResponseType registerExtendedConsent(
    ...
    // Typ av intyg som ger direktåtkomst till information från andra vårdgivare enligt PDL - fast
    // värde.
    ...
    registerExtendedConsentType.setPatientId(
        SamtyckestjanstUtil.buildIITypeForPersonOrSamordningsnummer(patientId));
    ...
    try {
      return registerExtendedConsentService.registerExtendedConsent(
          logicalAddress, registerExtendedConsentType);
    } catch (Exception e) {
      LOG.error(
          "SAMTYCKESTJÄNSTEN: Failure when calling Samtyckestjänsten and the registerExtendedConsent service.");
      throw new SamtyckestjanstIntegrationException(e.getMessage());
    }
  }
  ...
  private Personnummer createPersonnummer(String str, String fieldName) {
    return Personnummer.createPersonnummer(str)
        .orElseThrow(
            () ->
                new IllegalArgumentException(
                    fieldName + " must be a valid personnummer or samordningsnummer"));
  }
}