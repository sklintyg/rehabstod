...
import se.inera.intyg.rehabstod.application.sjukfall.SjukfallController;
...
class SjukfallControllerTest {
  ...
  private final List<Vardenhet> vardenheter =
      new ArrayList<>() {
        {
  ...
  private final List<Vardgivare> vardgivare =
      new ArrayList<>() {
        {
  ...
  class GetSjukfallByPatientTests {
    ...
    void getSjukfallByPatientWhenIntygFromSingleUnit() {
      ...
      try (MockedStatic<PDLActivityStore> pdlActivityStore = mockStatic(PDLActivityStore.class)) {
        ...
        verify(sjukfallService)
            .getByPatient(
                eq(DEFAULT_VARDGIVARE.getId()),
                eq(DEFAULT_VARDENHET.getId()),
                isNull(),
                eq(personnummer.getPersonnummer()),
                any(Urval.class),
                any(IntygParametrar.class),
                anyCollection(),
                anyCollection());

        pdlActivityStore.verify(
            () ->
  ...
  class PdlLogTests {
    ...
    void pdlLogSjukfallPatientShouldTriggerSRSLogEntryIfFeatureIsActive() {
      ...
      verify(logService, times(1))
          .logSjukfallData(
              any(PatientData.class),
              eq(ActivityType.READ),
              eq(ResourceType.RESOURCE_TYPE_PREDIKTION_SRS));
    }
    ...
    void pdlLogSjukfallPatientShouldNotTriggerSRSLogEntryIfFeatureISInactive() {
      ...
      final var sjukfallPatient1 =
          createSjukFallPatient(patientId, 1, 0, DEFAULT_VARDENHET, DEFAULT_VARDGIVARE);
      final var sjukfallPatient2 =
          createSjukFallPatient(patientId, 3, 2, DEFAULT_VARDENHET, DEFAULT_VARDGIVARE);
  ...
  private void assertPdlActivityEntries(
      RehabstodUser user, Vardenhet vardenhet, ResourceType resourceType) {
    final var pdlActivityEntries = user.getStoredActivities().get(vardenhet.getId());

    // Expect one INTYG with a certain ResourceType
    assertEquals(
        1L,
        pdlActivityEntries.stream()
  ...
  private static SjukfallEnhet createSjukFallEnhet(String personNummer) {
    ...
    // Not really interested in these properties, but the sjukfall equals /hashcode will fail
    // without them
  ...
  private SjukfallPatient createSjukFallPatient(
      String personNummer,
      int numberOfIntyg,
      int numberOfIntygHavingRisk,
      Vardenhet vardenhet,
