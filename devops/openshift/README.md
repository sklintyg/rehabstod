...
##### Create file secret and config map
    ...
    oc create configmap "rehabstod-test-config" --from-file=test/config/
    oc create secret generic "rehabstod-test-env" --from-file=test/env/ --type=Opaque
    oc create secret generic "rehabstod-test-certifikat" --from-file=test/certifikat/ --type=Opaque
...
##### Create file secret and config map
    ...
    oc create configmap "rehabstod-devtest-config" --from-file=devtest/config/
    oc create secret generic "rehabstod-devtest-env" --from-file=devtest/env/ --type=Opaque
...
Certificates are typically pre-installed into the devtest "project". If necessary, the JKS / P12 files can be copied into _devtest/certifikat_ and then installed into the project:    
    ...
    oc create secret generic "rehabstod-devtest-certifikat" --from-file=devtest/certifikat/ --type=Opaque
...
Note that the deploytemplate may be pre-installed in the OCP cluster.
    ...
    -p APP_NAME=rehabstod-devtest \
    -p IMAGE=docker-registry.default.svc:5000/dintyg/rehabstod-verified:latest \
    -p STAGE=devtest -p DATABASE_NAME=rehabstoddevtest \
