...
public class RehabstodUserDetailsService extends BaseUserDetailsService {
  ...
  public RehabstodUser buildUserPrincipal(
    ...
    // All rehab customization is done in the overridden decorateXXX methods, so just return a new
    // rehabuser
    IntygUser intygUser = super.buildUserPrincipal(employeeHsaId, authenticationScheme);
    RehabstodUser rehabstodUser =
        new RehabstodUser(intygUser, isPdlConsentGiven(intygUser.getHsaId()), intygUser.isLakare());
      ...
      if (!rehabstodUnitChangeService.changeValdVardenhet(
        ...
        final AnvandarPreference defaultUnitPref =
            anvandarPreferenceRepository.findByHsaIdAndKey(
                rehabstodUser.getHsaId(), Preference.DEFAULT_LOGIN_HSA_UNIT_ID.getBackendKeyName());
    ...
    // INTYG-5068: Explicitly changing vardenhet on session creation to possibly appyl
    // REHABKOORDINATOR role for
    // this unit in case the user is LAKARE and has systemRole Rehab- for the current unit.
    // This is only performed if there were a unit selected, e.g. user only has access to a single
    // unit.
  ...
  void removeEnheterMissingRehabKoordinatorRole(
    ...
    authorizedVardgivare.stream()
        .forEach(
            vg ->
                vg.getVardenheter().removeIf(ve -> !rehabAuthorizedEnhetIds.contains(ve.getId())));

    // Also, any vardgivare with no vardenheter left should be removed for the default selection
    // mechanism to work
