...
public final class RehabstodUserPreferences implements Serializable {
  ...
  public void validate() {
    ...
    validateIntRange(
        Preference.MAX_ANTAL_DAGAR_SEDAN_SJUKFALL_AVSLUT, MIN_DAYS, MAX_DAYS_SJUKLFALL);
  }
  ...
  private void validateIntRange(Preference p, int min, int max) {
    ...
    if (value < min || value > max) {
      throw new IllegalArgumentException(
          "Input (" + p.getBackendKeyName() + ") out of range: " + value);
    }
  }
  ...
  public enum Preference {
    ...
    MAX_ANTAL_DAGAR_SEDAN_SJUKFALL_AVSLUT(
        "maxAntalDagarSedanSjukfallAvslut", "maxAntalDagarSedanSjukfallAvslut", "0"),
    ...
    LAKARUTLATANDEN_TABLE_COLUMNS(
        "lakarutlatanden_table_columns", "lakarutlatandenTableColumns", ""),
    LAKARUTLATANDE_UNIT_TABLE_COLUMNS(
        "lakarutlatande_unit_table_columns", "lakarutlatandeUnitTableColumns", "");
